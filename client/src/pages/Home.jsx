...
export default function Home() {
  ...
  const lastThree = works.slice(-3);
  const earlier = works.slice(0, Math.max(0, works.length - 3));
  const slides = [...lastThree, ...earlier].slice(0, 8);
  const selected = works.filter((work) => work.id !== slides[0]?.id).slice(0, 4);
  ...
  const eyebrow = `${t("home.eyebrow")}${location ? ` · ${location}` : ""}`;
